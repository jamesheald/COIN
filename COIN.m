...
%             objectiveEst = -(log(1/obj.R) + obj.log_sum_exp(logLikelihood));
        ...
        function iResamp = resample_indices(obj,S)
            ...
            if ~isfield(S{1},'iResamp')
                error('The plot_COIN method cannot be used as no plot flags were activated in properties prior to calling the run_COIN method.')
            end
        ...
        function generate_figures(obj,P)
            ...
            if obj.cPredPlot
                ...
                for context = 1:max(P.posteriorMode)
                    c = P.cPred(:,context);
                    t = find(~isnan(P.cPred(:,context)),1,'first');
                    c(t-1) = P.cPred(t-1,end);
                    plot(c,'Color',RGB(context,:),'LineWidth',LW)
                end
