...
```
These properties must be set before running the model so that the relevant variables can be stored for plotting. For information on which other variables are available for plotting and how to plot them, see [Properties](#properties).
