...
Online inference can be performed without storing in memory all the past values of all variables inferred by the COIN model, and so to reduce memory requirements, the past values of variables are only stored if they need to be analysed. To store a variable on all trials, add the name of this variable to the store property. For example, to store the Kalman gains and responsibilities:
```
obj.store = {'k','cPost'};
```
