...
```
This returns a stochastic estimate of the objective. It is stochastic as it is derived from simulations that are conditioned on random observation noise. To reduce the variance of this estimate and aid parameter optimisation, the number of runs used to estimate the objective can be increased via the property R (this is best done with [Parallel Computing](#parallel-computing) to ensure acceptable runtimes). This estimate can be passed to an optimiser. It is important to use an optimiser that can handle a stochastic objective function (e.g. [BADS](https://github.com/lacerbi/bads)).
