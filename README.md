...
```
This returns a stochastic estimate of the objective as it is derived from simulations that are conditioned on random observation noise. To reduce the variance of this estimate (to aid parameter optimisation), increase the number of runs used to estimate the objective via the property R. It is important to use an optimiser that can handle a stochastic objective function (e.g. [BADS](https://github.com/lacerbi/bads)).
